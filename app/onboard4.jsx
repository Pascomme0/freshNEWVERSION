...
export default function Index() {
  ...
  return (
    <SafeAreaProvider>
      <View style={{ paddingTop: inset.top }} className="flex flex-col   bg-blue-100 h-screen">
        <View className="w-full h-[60%] flex items-center ">
          <ImageBackground resizeMode="cover" className="w-full h-full items-center" source={require('@/assets/images/deliver.png')}></ImageBackground>
        </View>
        <View className=" h-[50%] flex flex-col bg-white rounded-t-2xl px-4 w-full">
          <View className="mt-10">
            <Text className="text-center text-[22px]  tracking-widest font-bold">
              Faites vous livrez à
            </Text>
            <Text className="text-center text-[22px]  tracking-widest font-bold ">
              domicile
            </Text>
          </View>
          <View className=" z-50 fixed flex flex-row  items-center mt-[20%]  pt-6 justify-around ">
          <Link replace href={"/onboard3"} asChild>
              <Pressable>
